...
import org.mob.reach_for_the_stars.block.custom.multiblock.T4RocketAssemblerControllerBlock;
...
import org.mob.reach_for_the_stars.recipe.T4RocketAssemblerRecipe;
import org.mob.reach_for_the_stars.screen.menu.T4RocketAssemblerControllerMenu;
...
public class T4RocketAssemblerControllerBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public T4RocketAssemblerControllerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.T4_ROCKET_ASSEMBLER_CONTROLLER_BE.get(), pPos, pBlockState);
    }
    ...
    protected final ContainerData data = new ContainerData() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> T4RocketAssemblerControllerBlockEntity.this.progress;
                case 1 -> T4RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> T4RocketAssemblerControllerBlockEntity.this.progress = value;
                case 1 -> T4RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS = value;
            }
        }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, T4RocketAssemblerControllerBlockEntity be) {
        ...
        if (state.getValue(T4RocketAssemblerControllerBlock.FORMED)) {
            be.processRecipe(level);
        }
    }

    private void updateStructure(Level level, BlockPos pos, BlockState state) {
        MultiblockPattern.MatchResult result = MultiblockPatterns.T4_ROCKET_ASSEMBLER.check(
                level, pos, state.getValue(T4RocketAssemblerControllerBlock.FACING)
        );

        boolean wasFormed = state.getValue(T4RocketAssemblerControllerBlock.FORMED);

        if (result.isFormed()) {
            if (!wasFormed) {
                level.setBlock(pos, state.setValue(T4RocketAssemblerControllerBlock.FORMED, true), 3);
            }
            ...
            if (wasFormed) {
                level.setBlock(pos, state.setValue(T4RocketAssemblerControllerBlock.FORMED, false), 3);
                unlinkHatches(level);
                this.progress = 0;
                setChanged();
            }
    ...
    private void linkHatches(Level level, BlockPos controllerPos, MultiblockPattern.MatchResult result) {
        ...
        // 'L' is Item Input Hatch
        for (BlockPos targetPos : result.getPositions('L')) {
        ...
        // 'M' is Item Output Hatch
        for (BlockPos targetPos : result.getPositions('M')) {
        ...
        // 'K' is Energy Input Hatch
        for (BlockPos targetPos : result.getPositions('K')) {
    ...
    private Optional<T4RocketAssemblerRecipe> getRecipe(SimpleContainer container, Level level) {
        if (container.isEmpty()) return Optional.empty();

        return level.getRecipeManager().getRecipeFor(T4RocketAssemblerRecipe.Type.INSTANCE, container, level);
    }
    ...
    private void processRecipe(Level level) {
        ...
        Optional<T4RocketAssemblerRecipe> match = getRecipe(container, level);
        ...
        T4RocketAssemblerRecipe recipe = match.get();
    ...
    private void consumeRecipeIngredients(T4RocketAssemblerRecipe recipe, IItemHandler inputHandler) {
    ...
    public Component getDisplayName() {
        return Component.translatable("block.reach_for_the_stars.t4_rocket_assembler_controller");
    }

    @Nullable
    @Override
    public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
        return new T4RocketAssemblerControllerMenu(pContainerId, pPlayerInventory, this, this.data);
    }
