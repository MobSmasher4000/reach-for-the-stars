...
import org.mob.reach_for_the_stars.block.custom.multiblock.OstrumDrillControllerBlock;
...
import org.mob.reach_for_the_stars.screen.menu.OstrumDrillControllerMenu;
...
public class OstrumDrillControllerBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public OstrumDrillControllerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.OSTRUM_DRILL_CONTROLLER_BE.get(), pPos, pBlockState);
    }
    ...
    protected final ContainerData data = new ContainerData() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> OstrumDrillControllerBlockEntity.this.progress;
                case 1 -> OstrumDrillControllerBlockEntity.this.MAX_PROGRESS;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> OstrumDrillControllerBlockEntity.this.progress = value;
                case 1 -> OstrumDrillControllerBlockEntity.this.MAX_PROGRESS = value;
            }
        }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, OstrumDrillControllerBlockEntity be) {
        ...
        if (state.getValue(OstrumDrillControllerBlock.FORMED)) {
            be.processRecipe(level);
        }
    }

    private void updateStructure(Level level, BlockPos pos, BlockState state) {
        MultiblockPattern.MatchResult result = MultiblockPatterns.OSTRUM_DRILL.check(
                level, pos, state.getValue(OstrumDrillControllerBlock.FACING)
        );

        boolean wasFormed = state.getValue(OstrumDrillControllerBlock.FORMED);

        if (result.isFormed()) {
            if (!wasFormed) {
                level.setBlock(pos, state.setValue(OstrumDrillControllerBlock.FORMED, true), 3);
            }
            ...
            if (wasFormed) {
                level.setBlock(pos, state.setValue(OstrumDrillControllerBlock.FORMED, false), 3);
                unlinkHatches(level);
                this.progress = 0;
                setChanged();
            }
    ...
    private Optional<OstrumDrillRecipe> getRecipe(FluidStack inputBuffer, Level level) {
        if (inputBuffer.isEmpty()) return Optional.empty();

        ResourceLocation currentDimension = level.dimension().location();

        return level.getRecipeManager().getAllRecipesFor(OstrumDrillRecipe.Type.INSTANCE)
                .stream()
    ...
    private void processRecipe(Level level) {
        ...
        Optional<OstrumDrillRecipe> match = getRecipe(currentInputFluid, level);
        ...
        OstrumDrillRecipe recipe = match.get();
    ...
    public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
        return new OstrumDrillControllerMenu(pContainerId, pPlayerInventory, this, this.data);
    }
