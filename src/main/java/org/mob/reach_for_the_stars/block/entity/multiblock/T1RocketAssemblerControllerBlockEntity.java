...
import org.mob.reach_for_the_stars.block.custom.multiblock.T1RocketAssemblerControllerBlock;
...
import org.mob.reach_for_the_stars.screen.menu.T1RocketAssemblerControllerMenu;
...
public class T1RocketAssemblerControllerBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public T1RocketAssemblerControllerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.T1_ROCKET_ASSEMBLER_CONTROLLER_BE.get(), pPos, pBlockState);
    }
    ...
    protected final ContainerData data = new ContainerData() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> T1RocketAssemblerControllerBlockEntity.this.progress;
                case 1 -> T1RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> T1RocketAssemblerControllerBlockEntity.this.progress = value;
                case 1 -> T1RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS = value;
            }
        }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, T1RocketAssemblerControllerBlockEntity be) {
        ...
        if (state.getValue(T1RocketAssemblerControllerBlock.FORMED)) {
            be.processRecipe(level);
        }
    }

    private void updateStructure(Level level, BlockPos pos, BlockState state) {
        MultiblockPattern.MatchResult result = MultiblockPatterns.T1_ROCKET_ASSEMBLER.check(
                level, pos, state.getValue(T1RocketAssemblerControllerBlock.FACING)
        );

        boolean wasFormed = state.getValue(T1RocketAssemblerControllerBlock.FORMED);

        if (result.isFormed()) {
            if (!wasFormed) {
                level.setBlock(pos, state.setValue(T1RocketAssemblerControllerBlock.FORMED, true), 3);
            }
            ...
            if (wasFormed) {
                level.setBlock(pos, state.setValue(T1RocketAssemblerControllerBlock.FORMED, false), 3);
                unlinkHatches(level);
                this.progress = 0;
                setChanged();
            }
    ...
    private Optional<T1RocketAssemblerRecipe> getRecipe(SimpleContainer container, Level level) {
        if (container.isEmpty()) return Optional.empty();

        return level.getRecipeManager().getRecipeFor(T1RocketAssemblerRecipe.Type.INSTANCE, container, level);
    }
    ...
    private void processRecipe(Level level) {
        ...
        Optional<T1RocketAssemblerRecipe> match = getRecipe(container, level);
        ...
        T1RocketAssemblerRecipe recipe = match.get();
    ...
    private void consumeRecipeIngredients(T1RocketAssemblerRecipe recipe, IItemHandler inputHandler) {
    ...
    public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
        return new T1RocketAssemblerControllerMenu(pContainerId, pPlayerInventory, this, this.data);
    }
