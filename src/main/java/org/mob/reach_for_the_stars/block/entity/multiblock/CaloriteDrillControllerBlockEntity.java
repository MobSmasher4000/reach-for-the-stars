...
import org.mob.reach_for_the_stars.block.custom.multiblock.CaloriteDrillControllerBlock;
...
import org.mob.reach_for_the_stars.recipe.CaloriteDrillRecipe;
import org.mob.reach_for_the_stars.screen.menu.CaloriteDrillControllerMenu;
...
public class CaloriteDrillControllerBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public CaloriteDrillControllerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.CALORITE_DRILL_CONTROLLER_BE.get(), pPos, pBlockState);
    }
    ...
    protected final ContainerData data = new ContainerData() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> CaloriteDrillControllerBlockEntity.this.progress;
                case 1 -> CaloriteDrillControllerBlockEntity.this.MAX_PROGRESS;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> CaloriteDrillControllerBlockEntity.this.progress = value;
                case 1 -> CaloriteDrillControllerBlockEntity.this.MAX_PROGRESS = value;
            }
        }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, CaloriteDrillControllerBlockEntity be) {
        ...
        if (state.getValue(CaloriteDrillControllerBlock.FORMED)) {
            be.processRecipe(level);
        }
    }

    private void updateStructure(Level level, BlockPos pos, BlockState state) {
        MultiblockPattern.MatchResult result = MultiblockPatterns.CALORITE_DRILL.check(
                level, pos, state.getValue(CaloriteDrillControllerBlock.FACING)
        );

        boolean wasFormed = state.getValue(CaloriteDrillControllerBlock.FORMED);

        if (result.isFormed()) {
            if (!wasFormed) {
                level.setBlock(pos, state.setValue(CaloriteDrillControllerBlock.FORMED, true), 3);
            }
            ...
            if (wasFormed) {
                level.setBlock(pos, state.setValue(CaloriteDrillControllerBlock.FORMED, false), 3);
                unlinkHatches(level);
                this.progress = 0;
                setChanged();
            }
    ...
    private Optional<CaloriteDrillRecipe> getRecipe(FluidStack inputBuffer, Level level) {
        if (inputBuffer.isEmpty()) return Optional.empty();

        ResourceLocation currentDimension = level.dimension().location();

        return level.getRecipeManager().getAllRecipesFor(CaloriteDrillRecipe.Type.INSTANCE)
                .stream()
    ...
    private void processRecipe(Level level) {
        ...
        Optional<CaloriteDrillRecipe> match = getRecipe(currentInputFluid, level);
        ...
        CaloriteDrillRecipe recipe = match.get();
    ...
    public Component getDisplayName() {
        return Component.translatable("block.reach_for_the_stars.calorite_drill_controller");
    }

    @Nullable
    @Override
    public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
        return new CaloriteDrillControllerMenu(pContainerId, pPlayerInventory, this, this.data);
    }
