...
import org.mob.reach_for_the_stars.block.custom.multiblock.DeshDrillControllerBlock;
...
import org.mob.reach_for_the_stars.recipe.DeshDrillRecipe;
import org.mob.reach_for_the_stars.screen.menu.DeshDrillControllerMenu;
...
public class DeshDrillControllerBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public DeshDrillControllerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.DESH_DRILL_CONTROLLER_BE.get(), pPos, pBlockState);
    }
    ...
    protected final ContainerData data = new ContainerData() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> DeshDrillControllerBlockEntity.this.progress;
                case 1 -> DeshDrillControllerBlockEntity.this.MAX_PROGRESS;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> DeshDrillControllerBlockEntity.this.progress = value;
                case 1 -> DeshDrillControllerBlockEntity.this.MAX_PROGRESS = value;
            }
        }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, DeshDrillControllerBlockEntity be) {
        ...
        if (state.getValue(DeshDrillControllerBlock.FORMED)) {
            be.processRecipe(level);
        }
    }

    private void updateStructure(Level level, BlockPos pos, BlockState state) {
        MultiblockPattern.MatchResult result = MultiblockPatterns.DESH_DRILL.check(
                level, pos, state.getValue(DeshDrillControllerBlock.FACING)
        );

        boolean wasFormed = state.getValue(DeshDrillControllerBlock.FORMED);

        if (result.isFormed()) {
            if (!wasFormed) {
                level.setBlock(pos, state.setValue(DeshDrillControllerBlock.FORMED, true), 3);
            }
            ...
            if (wasFormed) {
                level.setBlock(pos, state.setValue(DeshDrillControllerBlock.FORMED, false), 3);
                unlinkHatches(level);
                this.progress = 0;
                setChanged();
            }
    ...
    private void linkHatches(Level level, BlockPos controllerPos, MultiblockPattern.MatchResult result) {
        ...
        // 'E' is Energy Input
        for (BlockPos targetPos : result.getPositions('E')) {
    ...
    private Optional<DeshDrillRecipe> getRecipe(FluidStack inputBuffer, Level level) {
        if (inputBuffer.isEmpty()) return Optional.empty();

        ResourceLocation currentDimension = level.dimension().location();

        return level.getRecipeManager().getAllRecipesFor(DeshDrillRecipe.Type.INSTANCE)
                .stream()
    ...
    private void processRecipe(Level level) {
        ...
        Optional<DeshDrillRecipe> match = getRecipe(currentInputFluid, level);
        ...
        DeshDrillRecipe recipe = match.get();
    ...
    public Component getDisplayName() {
        return Component.translatable("block.reach_for_the_stars.desh_drill_controller");
    }

    @Nullable
    @Override
    public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
        return new DeshDrillControllerMenu(pContainerId, pPlayerInventory, this, this.data);
    }
