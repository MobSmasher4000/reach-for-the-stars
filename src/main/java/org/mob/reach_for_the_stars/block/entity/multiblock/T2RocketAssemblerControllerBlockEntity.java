...
import org.mob.reach_for_the_stars.block.custom.multiblock.T2RocketAssemblerControllerBlock;
...
import org.mob.reach_for_the_stars.recipe.T2RocketAssemblerRecipe;
import org.mob.reach_for_the_stars.screen.menu.T2RocketAssemblerControllerMenu;
...
public class T2RocketAssemblerControllerBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public T2RocketAssemblerControllerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.T2_ROCKET_ASSEMBLER_CONTROLLER_BE.get(), pPos, pBlockState);
    }
    ...
    protected final ContainerData data = new ContainerData() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> T2RocketAssemblerControllerBlockEntity.this.progress;
                case 1 -> T2RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> T2RocketAssemblerControllerBlockEntity.this.progress = value;
                case 1 -> T2RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS = value;
            }
        }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, T2RocketAssemblerControllerBlockEntity be) {
        ...
        if (state.getValue(T2RocketAssemblerControllerBlock.FORMED)) {
            be.processRecipe(level);
        }
    }

    private void updateStructure(Level level, BlockPos pos, BlockState state) {
        MultiblockPattern.MatchResult result = MultiblockPatterns.T2_ROCKET_ASSEMBLER.check(
                level, pos, state.getValue(T2RocketAssemblerControllerBlock.FACING)
        );

        boolean wasFormed = state.getValue(T2RocketAssemblerControllerBlock.FORMED);

        if (result.isFormed()) {
            if (!wasFormed) {
                level.setBlock(pos, state.setValue(T2RocketAssemblerControllerBlock.FORMED, true), 3);
            }
            ...
            if (wasFormed) {
                level.setBlock(pos, state.setValue(T2RocketAssemblerControllerBlock.FORMED, false), 3);
                unlinkHatches(level);
                this.progress = 0;
                setChanged();
            }
    ...
    private void linkHatches(Level level, BlockPos controllerPos, MultiblockPattern.MatchResult result) {
        ...
        // 'J' is Energy Input Hatch
        for (BlockPos targetPos : result.getPositions('J')) {
    ...
    private Optional<T2RocketAssemblerRecipe> getRecipe(SimpleContainer container, Level level) {
        if (container.isEmpty()) return Optional.empty();

        return level.getRecipeManager().getRecipeFor(T2RocketAssemblerRecipe.Type.INSTANCE, container, level);
    }
    ...
    private void processRecipe(Level level) {
        ...
        Optional<T2RocketAssemblerRecipe> match = getRecipe(container, level);
        ...
        T2RocketAssemblerRecipe recipe = match.get();
    ...
    private void consumeRecipeIngredients(T2RocketAssemblerRecipe recipe, IItemHandler inputHandler) {
    ...
    public Component getDisplayName() {
        return Component.translatable("block.reach_for_the_stars.t2_rocket_assembler_controller");
    }

    @Nullable
    @Override
    public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
        return new T2RocketAssemblerControllerMenu(pContainerId, pPlayerInventory, this, this.data);
    }
