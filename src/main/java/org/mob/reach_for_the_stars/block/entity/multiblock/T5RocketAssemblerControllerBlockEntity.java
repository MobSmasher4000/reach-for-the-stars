...
import org.mob.reach_for_the_stars.block.custom.multiblock.T5RocketAssemblerControllerBlock;
...
import org.mob.reach_for_the_stars.recipe.T5RocketAssemblerRecipe;
...
public class T5RocketAssemblerControllerBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public T5RocketAssemblerControllerBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.T5_ROCKET_ASSEMBLER_CONTROLLER_BE.get(), pPos, pBlockState);
    }
    ...
    protected final ContainerData data = new ContainerData() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> T5RocketAssemblerControllerBlockEntity.this.progress;
                case 1 -> T5RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> T5RocketAssemblerControllerBlockEntity.this.progress = value;
                case 1 -> T5RocketAssemblerControllerBlockEntity.this.MAX_PROGRESS = value;
            }
        }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, T5RocketAssemblerControllerBlockEntity be) {
        ...
        if (state.getValue(T5RocketAssemblerControllerBlock.FORMED)) {
            be.processRecipe(level);
        }
    }

    private void updateStructure(Level level, BlockPos pos, BlockState state) {
        MultiblockPattern.MatchResult result = MultiblockPatterns.T5_ROCKET_ASSEMBLER.check(
                level, pos, state.getValue(T5RocketAssemblerControllerBlock.FACING)
        );

        boolean wasFormed = state.getValue(T5RocketAssemblerControllerBlock.FORMED);

        if (result.isFormed()) {
            if (!wasFormed) {
                level.setBlock(pos, state.setValue(T5RocketAssemblerControllerBlock.FORMED, true), 3);
            }
            ...
            if (wasFormed) {
                level.setBlock(pos, state.setValue(T5RocketAssemblerControllerBlock.FORMED, false), 3);
                unlinkHatches(level);
                this.progress = 0;
                setChanged();
            }
    ...
    private void linkHatches(Level level, BlockPos controllerPos, MultiblockPattern.MatchResult result) {
        ...
        // 'N' is Item Input Hatch
        for (BlockPos targetPos : result.getPositions('N')) {
        ...
        // 'O' is Item Output Hatch
        for (BlockPos targetPos : result.getPositions('O')) {
    ...
    private Optional<T5RocketAssemblerRecipe> getRecipe(SimpleContainer container, Level level) {
        if (container.isEmpty()) return Optional.empty();

        return level.getRecipeManager().getRecipeFor(T5RocketAssemblerRecipe.Type.INSTANCE, container, level);
    }
    ...
    private void processRecipe(Level level) {
        ...
        Optional<T5RocketAssemblerRecipe> match = getRecipe(container, level);
        ...
        T5RocketAssemblerRecipe recipe = match.get();
    ...
    private void consumeRecipeIngredients(T5RocketAssemblerRecipe recipe, IItemHandler inputHandler) {
    ...
    public Component getDisplayName() {
        return Component.translatable("block.reach_for_the_stars.t5_rocket_assembler_controller");
    }

    @Nullable
    @Override
    public AbstractContainerMenu createMenu(int pContainerId, Inventory pPlayerInventory, Player pPlayer) {
        return new T5RocketAssemblerControllerMenu(pContainerId, pPlayerInventory, this, this.data);
    }
